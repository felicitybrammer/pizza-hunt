...
request.onupgradeneeded = function(event) {
    ...
    //create an object store (table) called `new_pizza` and set it to have
...
function uploadPizza() {
    ...
    getAll.onsuccess = function() {
        //if there was data in indexedDB's store, send it to api
        if (getAll.result.length > 0) {
            fetch('/api/pizzas', {
                ...
                .then(serverResponse => {
                    ...
                    alert('All saved pizzed has been submitted!');
                })
                .catch( err => {
                    console.log(err);
                });
